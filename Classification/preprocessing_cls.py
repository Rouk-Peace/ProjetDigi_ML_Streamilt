    ...
    try:
        ...
        if selected_columns:
            ...
            with st.expander("Nettoyage des Données", expanded=True):
                ...
                st.write("**Remplacement de valeurs spécifiques :**")
                column_to_replace = st.selectbox("Sélectionnez la colonne pour remplacer les valeurs", df.columns)
                value_to_replace = st.text_input("Valeur à remplacer", "Vin éuilibré")
                replacement_value = st.text_input("Nouvelle valeur", "Vin équilibré")
                
                if st.button("Remplacer les valeurs"):
                    df[column_to_replace] = df[column_to_replace].replace(value_to_replace, replacement_value)
                    st.write(f"Valeurs '{value_to_replace}' remplacées par '{replacement_value}' dans la colonne '{column_to_replace}'.")
                ...
                encoding_option = st.selectbox(
                    ...
                    ["Aucune", "Encodage One-Hot", "Encodage LabelEncoder"]
                )
                ...
                elif encoding_option == "Encodage LabelEncoder":
                    if st.button("Encoder les variables catégorielles avec LabelEncoder"):
                        le = LabelEncoder()
                        for col in categorical_columns:
                            df[col] = le.fit_transform(df[col])
                        st.write("Variables catégorielles encodées avec LabelEncoder.")
